...
export default function App() {
  ...
  useEffect(()=>{
    loadPersonas().then(saved=>{
      ...
      // 초기 화면은 replaceState로 기록 — 여기서 뒤로가기 시 앱 종료(정상 동작)
      window.history.replaceState({screen: initial, activeId: null}, "");
      setScreen(initial);
    });
  },[]);
  ...
  useEffect(()=>{
    function onPop(e: PopStateEvent) {
      const st = e.state as {screen:"loading"|"landing"|"list"|"setup"|"chat"; activeId:string|null} | null;
  ...
  function navigate(next:"loading"|"landing"|"list"|"setup"|"chat", nextActiveId: string|null = activeId) {
  ...
  if(screen==="list") return <>{fl}<PersonaList personas={personas} activeId={activeId} onSelect={id=>navigate("chat",id)} onCreate={()=>navigate("setup")} onBack={()=>navigate("landing")} onDelete={deletePersona}/></>;
