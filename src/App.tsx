...
// ── 사용자 Gemini API 키 관리 ──
// 서버에 키를 두지 않고, 사용자가 직접 발급받은 키를 이 기기에만 저장해 본인 명의로 API를 호출합니다.
let userGeminiKey = "";
function setUserGeminiKey(key: string): void { userGeminiKey = key.trim(); }
function hasUserGeminiKey(): boolean { return userGeminiKey.length>0; }
async function loadUserGeminiKey(): Promise<string> {
  try {
    const r = await storageGet(API_KEY_STORAGE_KEY, false);
    const k = r?.value ?? "";
    userGeminiKey = k;
    return k;
  } catch { return ""; }
}
async function saveUserGeminiKey(key: string): Promise<void> {
  setUserGeminiKey(key);
  await storageSet(API_KEY_STORAGE_KEY, key.trim(), false);
}
async function clearUserGeminiKey(): Promise<void> {
  setUserGeminiKey("");
  await storageSet(API_KEY_STORAGE_KEY, "", false);
}
class ApiKeyMissingError extends Error {}

// ── Gemini API — 사용자 본인의 키로 직접 호출 ──
function callGemini(system: string, msgs: ClaudeMsg[]): Promise<string> {
  if (!hasUserGeminiKey()) return Promise.reject(new ApiKeyMissingError("no api key"));
    ...
    fetch(url, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body) })
      .then(r => {
        ...
        if (r.status === 400 || r.status === 403) {
          return Promise.reject(new ApiKeyMissingError("invalid api key"));
        }
...
// ── ApiKeySetup — 사용자 본인의 Gemini API 키 입력/저장 ──
const API_KEY_GUIDE_STEPS = [
  "aistudio.google.com/apikey 접속 후 구글 계정으로 로그인",
  "\"Create API key\"(API 키 만들기) 버튼 클릭",
  "새 프로젝트를 만들거나 기존 프로젝트를 선택",
  "생성된 키(AIza로 시작하는 문자열)를 복사",
  "아래 입력창에 붙여넣고 저장",
];

function ApiKeySetup({allowSkip,onSaved,onSkip}:{allowSkip:boolean;onSaved:()=>void;onSkip:()=>void}) {
  const [value,setValue]=useState(userGeminiKey);
  ...
  async function handleSave() {
    ...
    await saveUserGeminiKey(trimmed);
    setSaving(false);
    onSaved();
  }
  ...
  return (
    <div style={s}>
      <Blobs/>
      <div style={{...glass,position:"relative",zIndex:1,borderRadius:20,padding:28,width:"100%",maxWidth:400}}>
        <div style={{fontWeight:700,fontSize:17,color:"#1a1a3e",marginBottom:10}}>Gemini API 키 설정</div>
        <div style={{fontSize:13,color:"#4a4a6a",lineHeight:1.8,marginBottom:18}}>
          SweetTalk API 버전은 서버 없이, 본인이 발급받은 Google Gemini API 키로 직접 대화를 생성합니다.<br/>
          입력한 키는 이 기기에만 저장되며 외부로 전송되지 않습니다.<br/>
          <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" style={{color:"#6a6aff",fontWeight:600}}>Google AI Studio에서 무료로 키 발급받기 ↗</a>
        </div>
        <button
          type="button"
          onClick={()=>setShowGuide(g=>!g)}
          aria-expanded={showGuide}
          style={{width:"100%",textAlign:"left",background:"rgba(255,255,255,0.5)",border:"1px solid rgba(106,143,255,0.25)",borderRadius:10,padding:"10px 12px",marginBottom:14,cursor:"pointer",fontSize:12.5,color:"#4a4a8a",fontWeight:600}}
        >
          {showGuide?"▾":"▸"} 발급 방법 자세히 보기
        </button>
        {showGuide&&(
          <ol style={{margin:"-6px 0 14px",paddingLeft:20,fontSize:12.5,color:"#4a4a6a",lineHeight:1.9}}>
            {API_KEY_GUIDE_STEPS.map((step,i)=><li key={i}>{step}</li>)}
          </ol>
        )}
        <div style={{position:"relative",marginBottom:8}}>
          <input
            ...
            style={{width:"100%",padding:"12px 44px 12px 14px",borderRadius:10,border:"1.5px solid rgba(106,143,255,0.3)",fontSize:14,outline:"none",background:"rgba(255,255,255,0.85)",boxSizing:"border-box"}}
          />
          <button onClick={()=>setShow(s=>!s)} aria-label={show?"키 숨기기":"키 표시"} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",fontSize:13,color:"#888",cursor:"pointer",padding:4}}>{show?"숨기기":"표시"}</button>
        </div>
        {error&&<div style={{fontSize:12,color:"#e05555",marginBottom:10}}>{error}</div>}
        <button onClick={handleSave} disabled={saving} style={{width:"100%",padding:"14px 0",borderRadius:14,border:"none",background:grad,color:"#fff",fontSize:15,cursor:saving?"default":"pointer",fontWeight:700,marginTop:8}}>{saving?"저장 중...":"저장하고 시작하기"}</button>
        <div style={{display:"flex",justifyContent:"space-between",marginTop:14}}>
          {userGeminiKey&&<button onClick={handleClear} style={{background:"none",border:"none",color:"#e05555",fontSize:12,cursor:"pointer"}}>키 삭제</button>}
          {allowSkip&&<button onClick={onSkip} style={{background:"none",border:"none",color:"#9999cc",fontSize:12,cursor:"pointer",marginLeft:"auto"}}>나중에 설정하기</button>}
        </div>
      </div>
    </div>
  );
...
// ── Landing ──
function Landing({onStart,onOpenApiKey}:{onStart:()=>void;onOpenApiKey:()=>void}) {
  const features = [
  ...
  return (
    <div style={s}>
      ...
      <div style={{position:"relative",zIndex:1,display:"flex",flexDirection:"column",alignItems:"center",width:"100%"}}>
        ...
        <button onClick={onOpenApiKey} style={{marginTop:16,background:"none",border:"none",color:"#7777bb",fontSize:12.5,cursor:"pointer",fontWeight:600}}>{hasUserGeminiKey()?"Gemini API 키 변경":"Gemini API 키 설정"}</button>
        <div style={{marginTop:20,fontSize:13,color:"#7777bb",textAlign:"center",lineHeight:1.8}}>설레는 대화로<br/>영어 실력이 자라요</div>
      </div>
    </div>
  );
}
...
// ── PersonaList ──
function PersonaList({personas,activeId,onSelect,onCreate,onBack,onDelete,onExport,onImport,showBackupReminder,onOpenApiKey}:{personas:Persona[];activeId:string|null;onSelect:(id:string)=>void;onCreate:()=>void;onBack:()=>void;onDelete:(id:string)=>void;onExport:()=>void;onImport:(file:File)=>void;showBackupReminder:boolean;onOpenApiKey:()=>void}) {
    ...
    <div style={{...BG,maxWidth:500,margin:"0 auto",fontFamily:DEFAULT_FONT_FAMILY}}>
      ...
      <div style={{...glass,borderBottom:"1px solid rgba(255,255,255,0.4)",padding:"16px 18px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <button onClick={onOpenApiKey} aria-label="API 키 설정" style={{background:"none",border:"none",fontSize:16,color:"#888",cursor:"pointer"}}>⚙️</button>
...
// ── SettingsTab ──
function SettingsTab({fontSize,setFontSize,fontFamily,setFontFamily,onClearChat,onClose,onOpenApiKey}:{fontSize:number;setFontSize:(f:number|((n:number)=>number))=>void;fontFamily:string;setFontFamily:(f:string)=>void;onClearChat:()=>void;onClose:()=>void;onOpenApiKey:()=>void}) {
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:500,padding:28,paddingBottom:36,fontFamily}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:28}}>
          <div style={{fontSize:14,fontWeight:600,color:"#555",marginBottom:14}}>Gemini API 키</div>
          <button onClick={onOpenApiKey} style={{width:"100%",padding:"12px 0",borderRadius:12,border:"1.5px solid rgba(106,143,255,0.3)",background:"#f5f5ff",cursor:"pointer",fontSize:14,color:"#3a5fff",fontFamily,fontWeight:600}}>
            {hasUserGeminiKey()?"API 키 변경":"API 키 설정"}
          </button>
        </div>
        {/* 폰트 선택 */}
        <div style={{marginBottom:28}}>
...
// ── Chat ──
function Chat({persona,onBack,updatePersona,onOpenApiKey}:{persona:Persona;onBack:()=>void;updatePersona:(id:string,fn:(p:Persona)=>Persona)=>void;onOpenApiKey:()=>void}) {
  ...
  const genReply=useCallback((msgs: Message[])=>{
    ...
    if(!hasUserGeminiKey()){
      updatePersona(p.id,pr=>({...pr,messages:[...pr.messages,{role:"ai",text:"(Gemini API 키가 설정되지 않았어요. 설정에서 키를 입력해주세요)",translation:"",alternatives:[]}]}));
      return;
    }
    setAiLoading(true);
    const history: ClaudeMsg[]=msgs.map(m=>({role:m.role==="ai"?"assistant":"user",content:m.text}));
    callGemini(buildSystem(p),history)
      .then(raw=>{
        ...
        setAiLoading(false);
        const msg = err instanceof ApiKeyMissingError
          ? "(API 키가 유효하지 않아요. 설정에서 키를 다시 확인해주세요)"
          : "(연결에 실패했어요. 잠시 후 다시 시도해주세요)";
        updatePersona(p.id,pr=>({...pr,messages:[...pr.messages,{role:"ai",text:msg,translation:"",alternatives:[]}]}));
      });
  },[updatePersona]);

  useEffect(()=>{
    if(persona.messages.length===0&&!aiLoading&&hasUserGeminiKey()){
      setAiLoading(true);
      callGemini(buildSystem(persona),[{role:"user",content:"안녕, 처음 만나서 반가워. 자기소개 해줘."}])
        .then(raw=>{
  ...
  const FAILURE_TEXTS = new Set(["(오프라인 상태예요. 네트워크 연결 후 다시 시도해주세요)","(응답을 받지 못했어요. 다시 시도해주세요)","(연결에 실패했어요. 잠시 후 다시 시도해주세요)","(API 키가 유효하지 않아요. 설정에서 키를 다시 확인해주세요)"]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#f0f2f5",maxWidth:500,margin:"0 auto",fontFamily}}>
      ...
      {showSettings&&<SettingsTab fontSize={fontSize} setFontSize={setFontSize} fontFamily={fontFamily} setFontFamily={setFontFamily} onClearChat={clearChat} onClose={()=>setShowSettings(false)} onOpenApiKey={onOpenApiKey}/>}
      {/* Header — 고정 */}
...
export default function App() {
  const [screen,setScreen]=useState<"loading"|"consent"|"apikey"|"landing"|"list"|"setup"|"chat">("loading");
  const [personas,setPersonas]=useState<Persona[]>([]);
  const [activeId,setActiveId]=useState<string|null>(null);
  const [lastExportAt,setLastExportAt]=useState<number|null>(null);
  const [apiKeyReturn,setApiKeyReturn]=useState<"landing"|"list"|"chat">("landing");
  function openApiKeySettings(returnTo:"landing"|"list"|"chat") {
    setApiKeyReturn(returnTo);
    navigate("apikey");
  }
  ...
  useEffect(()=>{
    ...
    loadPersonas().then(async saved=>{
      ...
      const savedKey = await loadUserGeminiKey();
      let first: "consent"|"apikey"|"landing"|"list"|"chat" = consent ? initial : "consent";
      if(consent && !savedKey) { first = "apikey"; setApiKeyReturn(initial==="list"?"list":"landing"); }
  ...
  function agreeConsent() {
    ...
    if(hasUserGeminiKey()) { navigate(personas.length>0 ? "list" : "landing"); return; }
    setApiKeyReturn(personas.length>0 ? "list" : "landing");
    navigate("apikey");
  }
  function afterApiKeySaved() {
    const target = apiKeyReturn==="chat" && activeId ? "chat" : apiKeyReturn==="list" ? "list" : "landing";
    navigate(target, activeId);
  }

  // Android/WebView 뒤로가기 버튼 → history.back() 대응
  useEffect(()=>{
    function onPop(e: PopStateEvent) {
      const st = e.state as {screen:"loading"|"consent"|"apikey"|"landing"|"list"|"setup"|"chat"; activeId:string|null} | null;
  ...
  function navigate(next:"loading"|"consent"|"apikey"|"landing"|"list"|"setup"|"chat", nextActiveId: string|null = activeId) {
  ...
  if(screen==="apikey") return <>{fl}<ApiKeySetup allowSkip={apiKeyReturn!=="chat"} onSaved={afterApiKeySaved} onSkip={()=>navigate(apiKeyReturn==="list"?"list":"landing")}/></>;
  if(screen==="landing") return <>{fl}<Landing onStart={()=>navigate("list")} onOpenApiKey={()=>openApiKeySettings("landing")}/></>;
  if(screen==="list") {
    ...
    return <>{fl}<PersonaList personas={personas} activeId={activeId} onSelect={id=>navigate("chat",id)} onCreate={()=>navigate("setup")} onBack={()=>navigate("landing")} onDelete={deletePersona} onExport={exportBackup} onImport={importBackup} showBackupReminder={showBackupReminder} onOpenApiKey={()=>openApiKeySettings("list")}/></>;
  }
  if(screen==="setup") return <>{fl}<SetupWizard onDone={handleSetupDone} onCancel={()=>navigate(personas.length>0?"list":"landing")}/></>;
  if(screen==="chat"&&active) return <>{fl}<Chat persona={active} onBack={()=>navigate("list")} updatePersona={updatePersona} onOpenApiKey={()=>openApiKeySettings("chat")}/></>;
  return null;
}
